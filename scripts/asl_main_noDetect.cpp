...
int main(int, char**) {
	...
	while (true) {
		...
		cropped_im= frame(cv::Range(50,300),cv::Range(50,330));
		cv::imshow("cropped", cropped_im);
		cv::Point2f forw = model.forward(cropped_im);
		...
		if(fingerCount.getBoundingRectArea() > 0) {
			...
			cv::Point2f forw = model.forward(pt);pt
			std::cout << classes[forw.x] << std::endl;
			std::cout << "\n" << std::endl;
			cv::imshow("cropped", cropped);
		} else {
			cv::imshow("cropped", frameOut);pt
		}
		*/
